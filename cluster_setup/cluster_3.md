...
Login as the root user from the commandline. In OSX and Linux systems this can be done remotely within a terminal with:
    ...
    ssh root@me344-cluster-3.stanford.edu
...
Add PBS Professional client support
    ...
    perl -pi -e "s/PBS_SERVER=\S+/PBS_SERVER=me344-cluster-3.stanford.edu/" /etc/pbs.conf
    perl -pi -e "s/PBS_SERVER=\S+/PBS_SERVER=me344-cluster-3.stanford.edu/" $CHROOT/etc/pbs.conf
    perl -pi -e "s/\$clienthost \S+/\$clienthost me344-cluster-3.stanford.edu/" $CHROOT/var/spool/pbs/mom_priv/config
...
Add nodes to Warewulf data store:
    ...
    wwsh -y node new compute-1-1 --ipaddr=10.1.1.10 --hwaddr=A4:BF:01:15:37:5F
    wwsh -y node new compute-1-2 --ipaddr=10.1.2.10 --hwaddr=A4:BF:01:15:3C:B4
    wwsh -y node new compute-1-3 --ipaddr=10.1.3.10 --hwaddr=A4:BF:01:15:39:A8
    wwsh -y node new compute-1-4 --ipaddr=10.1.4.10 --hwaddr=A4:BF:01:15:32:BE

    wwsh -y node new ipmi-compute-1-1 --ipaddr=10.1.1.11 --hwaddr=A4:BF:01:15:37:61
    wwsh -y node new ipmi-compute-1-2 --ipaddr=10.1.2.11 --hwaddr=A4:BF:01:15:3C:B6
    wwsh -y node new ipmi-compute-1-3 --ipaddr=10.1.3.11 --hwaddr=A4:BF:01:15:39:AA
    wwsh -y node new ipmi-compute-1-4 --ipaddr=10.1.4.11 --hwaddr=A4:BF:01:15:32:C0
