...
- Set enp7s0 to `ON`
- Set enp8s0 to `ON`
- Set hostname to `me344-cluster-4.stanford.edu`
...
Login as the root user from the commandline. In OSX and Linux systems this can be done remotely within a terminal with:
    ...
    ssh root@me344-cluster-4.stanford.edu
...
Edit /etc/hosts -- Add in entry for public/private hostname mapping to IP address
    ...
    echo "171.64.116.190 me344-cluster-4.stanford.edu" >> /etc/hosts
    echo "10.1.1.1 me344-cluster-4.localdomain me344-cluster-4" >> /etc/hosts

Add PBS Professional client support

    yum -y --installroot=$CHROOT install pbspro-execution-ohpc
    perl -pi -e "s/PBS_SERVER=\S+/PBS_SERVER=me344-cluster-4.stanford.edu/" /etc/pbs.conf
    perl -pi -e "s/PBS_SERVER=\S+/PBS_SERVER=me344-cluster-4.stanford.edu/" $CHROOT/etc/pbs.conf
    perl -pi -e "s/\$clienthost \S+/\$clienthost me344-cluster-4.stanford.edu/" $CHROOT/var/spool/pbs/mom_priv/config
...
Add nodes to Warewulf data store:
    ...
    wwsh -y node new compute-1-1 --ipaddr=10.1.1.10 --hwaddr=A4:BF:01:15:3A:CA
    wwsh -y node new compute-1-2 --ipaddr=10.1.2.10 --hwaddr=A4:BF:01:15:36:0B
    wwsh -y node new compute-1-3 --ipaddr=10.1.3.10 --hwaddr=A4:BF:01:15:35:ED
    wwsh -y node new compute-1-4 --ipaddr=10.1.4.10 --hwaddr=A4:BF:01:15:39:9E

    wwsh -y node new ipmi-compute-1-1 --ipaddr=10.1.1.11 --hwaddr=A4:BF:01:15:3A:CC
    wwsh -y node new ipmi-compute-1-2 --ipaddr=10.1.2.11 --hwaddr=A4:BF:01:15:36:0D
    wwsh -y node new ipmi-compute-1-3 --ipaddr=10.1.3.11 --hwaddr=A4:BF:01:15:35:EF
    wwsh -y node new ipmi-compute-1-4 --ipaddr=10.1.4.11 --hwaddr=A4:BF:01:15:39:A0
